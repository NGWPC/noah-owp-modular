 module noahowp_log_module
  ...
  subroutine get_log_file_name()
    ...
    if (trim(adjustl(log_file_path)) == "") then
      ...
      if (status == 0) then
        ...
        log_file_path = trim(temp_path)// '/' // 'noahowp_log_' // time_stamp(12:13)//time_stamp(15:16)//time_stamp(18:19)//'.txt'
  ...
  subroutine create_logger(log_level_str)
    ...
    call get_environment_variable("NOAHOWP_LOGLEVEL", this_log_level_str)
