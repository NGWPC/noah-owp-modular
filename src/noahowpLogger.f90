module noahowp_log_module
...
#ifdef NOAHOWP_USE_EWTS
  character(len=*), parameter, public :: LOG_MODULE_ID = EWTS_ID_NOAH_OWP_MODULAR
...
#else
  character(len=*), parameter, public :: LOG_MODULE_ID = "NOAHOWP"
  ...
  logical, parameter :: NOAHOWP_FALLBACK_LOGGING_ENABLED = .true.
  integer, parameter :: NOAHOWP_FALLBACK_LOG_LEVEL = LOG_LEVEL_INFO
#endif
...
contains
...
#ifdef NOAHOWP_USE_EWTS
    call ensure_init()
    call ewts_write_log_module(EWTS_ID_NOAH_OWP_MODULAR, trim(message), mapped_level)
#else
    if (.not. is_logger_enabled()) return
    if (mapped_level < NOAHOWP_FALLBACK_LOG_LEVEL) return

    call get_utc_timestamp(timestamp)
    write(*, '(A,1X,A,1X,A,1X,A)') &
      trim(timestamp), "NOAHOWP", trim(level_to_string(mapped_level)), trim(message)
    flush(6)
#endif
  ...
  subroutine payload_status(status, prog, msg, modnm)
    ...
    if (len_trim(modnm) > 0) then
        ...
        payload_ewts_id = trim(LOG_MODULE_ID)
    end if
...
#else
    is_logger_enabled = NOAHOWP_FALLBACK_LOGGING_ENABLED
#endif
...
#else
    get_log_level = NOAHOWP_FALLBACK_LOG_LEVEL
#endif
